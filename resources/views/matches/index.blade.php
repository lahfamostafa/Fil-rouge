...
@section('content')
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <span class="text-sm text-gray-500">{{ $matches->total() }} match{{ $matches->total() !== 1 ? 'es' : '' }} available</span>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if(session('error'))
                <div class="mb-6 flex items-center gap-3 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-xl shadow-sm">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($matches as $match)
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col overflow-hidden">
                            ...
                            {{-- Card Header --}}
                            <div class="bg-gradient-to-r from-green-500 to-emerald-600 px-5 py-4 flex items-center justify-between">
                                ...
                                @elseif($match->status === 'full')
                                    <span class="inline-flex items-center gap-1 bg-white/20 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                                        <span class="w-1.5 h-1.5 rounded-full bg-yellow-300"></span> Full
                                    </span>
                                @else
                                    <span class="inline-flex items-center gap-1 bg-white/20 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                            ...
                            <div class="p-5 flex flex-col flex-1 gap-4">
                                ...
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-semibold text-gray-700">{{ $match->creator->name ?? 'Unknown' }}</p>
                                    </div>
                                </div>
                                ...
                                <div>
                                    <div class="flex justify-between text-xs text-gray-500 mb-1.5">
                                        <span>Players</span>
                                        <span class="font-semibold {{ $acceptedCount >= $match->max_players ? 'text-red-500' : 'text-green-600' }}">
                                ...
                                <div class="flex gap-2 mt-auto pt-2">
                                    ...
                                    @elseif($hasJoined)
                                        <span class="flex-1 text-center text-sm font-semibold text-gray-400 bg-gray-50 border border-gray-200 px-3 py-2 rounded-lg">
                                            Joined
                                        </span>
                                    @elseif($isFull && !$isMine && !$hasJoined)
                                        <span class="flex-1 text-center text-sm font-semibold text-red-400 bg-red-50 border border-red-200 px-3 py-2 rounded-lg">
                                            Full
                                        </span>
                                    @endif
                                </div>
