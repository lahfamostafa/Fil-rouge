...
<div class="flex h-[calc(100vh-56px)] overflow-hidden">
    ...
    <aside class="hidden sm:flex w-64 bg-white border-r border-slate-100 flex-col shrink-0">
        ...
        <nav class="flex-1 px-3 space-y-0.5 overflow-y-auto">
            ...
            <a href="/matches"
               class="sidebar-link {{ request()->is('matches*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all group">
                <span class="w-8 h-8 rounded-lg bg-slate-100 group-hover:bg-green-50 flex items-center justify-center shrink-0 transition-colors {{ request()->is('matches*') ? '!bg-white/20' : '' }}">
                    <svg class="w-4 h-4 text-slate-500 group-hover:text-green-600 {{ request()->is('matches*') ? '!text-white' : '' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                    </svg>
                </span>
                Matches
            ...
            <a href="/announcements"
               class="sidebar-link {{ request()->is('announcements*') ? 'active' : '' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all group">
                <span class="w-8 h-8 rounded-lg bg-slate-100 group-hover:bg-green-50 flex items-center justify-center shrink-0 transition-colors {{ request()->is('announcements*') ? '!bg-white/20' : '' }}">
                    <svg class="w-4 h-4 text-slate-500 group-hover:text-green-600 {{ request()->is('announcements*') ? '!text-white' : '' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
