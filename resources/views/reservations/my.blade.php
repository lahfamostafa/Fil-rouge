    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-4 hover:shadow-md hover:shadow-slate-100 transition-all">
        <div class="flex items-center gap-4">
            ...
            <div class="flex items-end gap-2 shrink-0">
                ...
                @endif
                <span class="px-2.5 py-1 rounded-lg text-xs font-bold text-center
                    {{ $res->status == 'pending' ? ' w-25 bg-amber-50 text-amber-700 border border-amber-200' : '' }}
                    {{ $res->status == 'confirmed' ? 'w-25 bg-green-50 text-green-700 border border-green-200' : '' }}
