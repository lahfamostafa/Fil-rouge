...
@section('content')
    ...
    @if (session('success'))
        <div
            class="flex items-center gap-3 bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-xl shadow-sm">
    ...
    @if (session('error'))
        <div
            class="flex items-center gap-3 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-xl shadow-sm mb-3">
                    ...
                    @if (auth()->user()->role === 'client')
                        ...
                        @if (!$res->match)
                            @if ($res->status == 'confirmed')
                                <a href="{{ route('matche.create', $res->id) }}"
                                    class="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 hover:text-white bg-emerald-50 hover:bg-emerald-600 border border-emerald-200 hover:border-emerald-600 px-3 py-1.5 rounded-xl transition-all duration-150 shrink-0">
                        ...
                        @else
                            <span
                                class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-xl shrink-0 cursor-default">
                                <svg class="w-3.5 h-3.5 text-green-400" fill="none" stroke="currentColor" stroke-width="2.5"
                                    viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        ...
                        @if (auth()->user()->role === 'manager' && $res->status === 'pending')
                            <form action="/manager/reservations/{{ $res->id }}/confirm" method="POST">
                                @csrf @method('PATCH')
                                <button
                                    class="text-xs text-green-600 hover:text-green-800 font-semibold transition-colors flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                    </svg>
                        ...
                        @if (auth()->user()->role === 'client' && $res->status !== 'cancelled' && !$res->match)
                            <form action="/reservations/{{ $res->id }}" method="POST">
                                @csrf @method('PATCH')
                                <button
                                    class="text-xs text-red-500 hover:text-red-700 font-semibold transition-colors flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2"
                                        viewBox="0 0 24 24">
